...
- **Ribbot trading bot wallet provisioning (Worker secrets)**:
  ...
  - FTX web authentication uses the Privy React SDK with the same public app ID as the Telegram bot. Telegram login recovers the existing Privy user and Solana wallet; new web users may explicitly create a wallet after login. Automatic wallet creation stays off so recovery cannot silently create a replacement. Public identifiers such as `NEXT_PUBLIC_PRIVY_APP_ID` are not secrets; never expose `PRIVY_APP_SECRET` or `PRIVY_AUTHORIZATION_PRIVATE_KEY` through `NEXT_PUBLIC_*`.
  ...
  - `ROBINHOOD_ALPHA_SCANNER_ENABLED=false` is the separate operator gate for the read-only Robinhood Chain research scanner. Keep it false without explicit activation approval. The scanner may read public market data and store public scores/signals, but it must never acquire wallet, Privy, swap, signing, broadcast, or automatic-trading authority.
