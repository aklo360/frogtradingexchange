...
- **Ribbot trading bot wallet provisioning (Worker secrets)**:
  ...
  - The FTX `/ribbot` client uses the matching public identifiers `NEXT_PUBLIC_PRIVY_APP_ID`, `NEXT_PUBLIC_PRIVY_BOT_SIGNER_ID`, and optional comma-separated `NEXT_PUBLIC_PRIVY_BOT_POLICY_IDS`. These IDs are not secrets; never expose `PRIVY_APP_SECRET` or `PRIVY_AUTHORIZATION_PRIVATE_KEY` through `NEXT_PUBLIC_*`. Privy Telegram login, the FTX production domain, and the Telegram bot domain must be configured in the same Privy app before live use.
  ...
  - Optional `RIBBOT_CONTROL_URL` is the FTX/FrogX browser URL Ribbot should display with short-lived account control codes; use `/ribbot` for the FTX-owned control page.
  - Optional `RIBBOT_WALLET_CLAIM_URL` is the Privy-authenticated app URL used for wallet handoff messaging. The `/ribbot` page configures `createOnLogin: "off"`, requires the authenticated Privy user ID, Telegram ID, and exact FTX wallet address to match, uses Privy's isolated export modal, and uses user-authorized signer removal/restoration. FTX control sessions record only non-secret lifecycle timestamps and must never receive or return exported key material.
...
- **`Profile`** (`/profile`): Wallet-connected Frog identity workspace using real Tapestry profile/social/recent-trade data and owned Solana NFTs, with distinct disconnected/loading/error/no-profile states and no synthetic points or rank.
- **`Ribbot`** (`/ribbot`): Flat, responsive Telegram account-control workspace. It exchanges Ribbot `/control` codes for FTX-owned control sessions, presents account/trading/wallet/list state in a scan-first desktop grid and linear mobile flow, manages simple/advanced mode, buy/sell presets, separate priority fees, confirmation/sell protection, and syncs FTX pause/restore state. A scoped Privy Telegram login independently gates wallet claim/export and app-signer removal/restoration by exact user/Telegram/wallet matching; export material never enters the page's FTX session or Worker responses.
