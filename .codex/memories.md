...
The FTX web app uses the Privy React SDK with the same public app ID as Ribbot. Profile offers Telegram-first recovery, lists the authenticated user's Solana wallets, and allows explicit wallet creation only when no wallet exists; automatic creation is off to avoid replacing a recovery target. Privy secrets remain Worker-only. FTX accepts the deployed `FROGX_BOT_API_TOKEN`/`PRIVY_SIGNER_ID` compatibility aliases. The target wallet `9p9UcNW4QaAcw6pRAMFtaJHuNChL6dFFnbYzARTnJSWY` was confirmed live on Solana on 2026-07-12; ownership recovery still requires the user to complete Privy Telegram login.
